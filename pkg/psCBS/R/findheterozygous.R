...
findheterozygous <- function(a.vector,b.vector,lower.bound=0,upper.bound=0.5,minimum.window=0.05,stepsize.window=0.01,stepsize.within=0.01)
  {
    if(minimum.window>(upper.bound-lower.bound)) stop ("Minimum window size is too big for the current lower.bound and upper.bound")
    min.vector <- apply(cbind(a.vector,b.vector),1,min)
    ...
    is.heterozygous <- rep(NA,length(min.vector))
    ...
    print(paste("Heterozygous cutoff is", round(heterozygous.cutoff,3)))
