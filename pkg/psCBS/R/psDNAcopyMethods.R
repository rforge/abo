...
#        if(p1!=p6) stop(paste("number of cols genomdat.a=",p1," number of cols normaldat.b=",p6))
    ...
    if(!is.null(normaldat.hetmatrix))
      {
        if(!is.logical(normaldat.hetmatrix)) stop("normaldat.hetmatrix must be of type logical (TRUE/FALSE)")
        if (is.vector(normaldat.hetmatrix)) normaldat.hetmatrix <- as.matrix(normaldat.hetmatrix)
        n8 <- nrow(normaldat.hetmatrix)
        p8 <- nrow(normaldat.hetmatrix)
        if(n1!=n8) stop(paste("number of rows normal.a=",n1," number of rows normaldat.hetmatrix=",n8))
#        if(p1!=p8) stop(paste("number of rows normal.a=",p1," number of rows normaldat.hetmatrix=",p8))
      }    
...
print.psCNA <- function(x, ...)
  {
    if (!inherits(x, 'psCNA')) stop("First arg must be of class psCNA")
