...
#Second round of segmentation
        if(matching.reference)
        ...
        else
          {
            ...
            write(reference.mbaf.heterozygous.notmissing,"reference-mbaf-heterozygous-notmissing.txt",ncol=1)
            diff.mbaf.vector.notmissing <- mbaf.vector.notmissing-reference.mbaf.heterozygous.notmissing
            write(diff.mbaf.vector.notmissing,"diff-mbaf-vector-notmissing.txt",ncol=1)
...
#        print("Before called segments")
        called.segments <- callsegments.matching(matching.reference,segment.starts,segment.ends,a.vector.notmissing,b.vector.notmissing,diff.mbaf.vector.notmissing,het.vector.notmissing,normal.het.vector.notmissing,chrom.vector.starts,total.vector.starts,min.hetero,trim.mean,alpha.equality,het.bias,1-maf,alpha.homozygous,min.homo.region,impute.LOH)
